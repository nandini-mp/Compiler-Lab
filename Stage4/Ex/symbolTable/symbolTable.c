...
Gsymbol* Install(char *name, int type, int size, int isPointer)
{
    if (Lookup(name)!=NULL)
    {
...
void parseDecl(tnode* root)
{
    ...
    switch(root->nodetype)
    {
        ...
        case Nvar:
        {
            Gsymbol* entry = Install(root->varname,dtype,1,root->isPointer);
        ...
        case Narr:
        {
            ...
            Gsymbol* entry = Install(root->left->varname,dtype,totalSize,root->isPointer);
