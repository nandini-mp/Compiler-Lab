...
struct tnode* makeVariableUseNode(char* name) {
	...
	temp->isPointer = symbol->isPointer;
...
{
	tnode* declAndBody = makeConnectNode(ldeclblock, body);
	tnode* paramlistAndRest = makeConnectNode(paramlist, declAndBody);
	tnode* idAndRest = makeConnectNode(id, paramlistAndRest);
	tnode* node = makeConnectNode(type,idAndRest);
	node->nodetype = Nfdef;
	node->varname = id->varname;
	return node;
}
...
{
	tnode* node = makeConnectNode(type,id);
...
tnode* makeFnCallNode(tnode* id, tnode* arglist)
{
	...
	tnode* node = makeConnectNode(id, arglist);
    node->nodetype = Nfcall;
    node->symbolTableEntry = entry;
    node->type = entry->type;   // function return type
    return node;
}