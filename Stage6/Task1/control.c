...
int codeGen(tnode* root)
{
	...
	switch (root->nodetype)
	{
		...
		case Nalloc:
		{
			...
			fprintf(target_file, "PUSH R%d\n", r5);       // unused
			fprintf(target_file, "CALL 0\n");
			fprintf(target_file, "MOV R%d, [SP-2]\n", r3); // read return value BEFORE popping
			fprintf(target_file, "SUB SP, 5\n");           // clean stack
		...
		case Nfree:
		{
			int addrReg = codeGen(root->left);   // get address to free
			int r1 = getFreeRegister();
			int r2 = getFreeRegister();
			int r3 = getFreeRegister();
			int r4 = getFreeRegister();
...
void generate(tnode* root) {
    ...
    // generate all function definitions including main
    codeGen(root);
}
