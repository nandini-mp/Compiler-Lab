...
program : TypeDefBlock GDeclBlock FDefBlock MainBlock { ASTRoot = makeConnectNode($1, makeConnectNode($2, makeConnectNode($3, $4))); }
        | TypeDefBlock GDeclBlock MainBlock { ASTRoot = makeConnectNode($1, makeConnectNode($2, $3)); }
        | TypeDefBlock MainBlock { ASTRoot = makeConnectNode($1, $2); }
        | GDeclBlock FDefBlock MainBlock { ASTRoot = makeConnectNode($1, makeConnectNode($2, $3)); }
        | GDeclBlock MainBlock { ASTRoot = makeConnectNode($1, $2); }
        | MainBlock { ASTRoot = $1; }
        ;
...
GDecl : TypeName {current_type = $1->typeEntry;} GidList SEMICOLON {$$ = makeDeclNode($1,$3);}
	;
