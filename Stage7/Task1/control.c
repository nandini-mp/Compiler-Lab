...
struct tnode *makeConstantNode(int n)
{
    ...
    temp->val = n;
    temp->type = 0;
    temp->typeEntry = TLookup("int");   // ← add this
    temp->varname = NULL;
...
int genAssignInstr(tnode* t)
{
    ...
    else if (t->left->nodetype == Nself)        // NEW
        addressReg = genSelfFieldAddress(t->left);
    else
...
int codeGen(tnode* root)
{
	...
	switch (root->nodetype)
	{
		...
		case Naddr :
		{
			int r = genAddressOfVar(root->left);
			return r;
		}
		case Nderef :
		{
		...
		case Nfdef :
		{
			...
			int fnLabel;
			if (root->symbolTableEntry != NULL) {
				fnLabel = root->symbolTableEntry->flabel;
			} else if (root->classEntry != NULL) {
				// class method — look up flabel from class table
				Cmethod* m = CMLookup(root->classEntry, root->varname);
				fnLabel = m ? m->flabel : -1;
			}
			else
				fnLabel = -1;
		...
		case Nself:
		{
			// load value of self.fieldname
			int addrReg = genSelfFieldAddress(root);
			int dataReg = getFreeRegister();
			fprintf(target_file, "MOV R%d, [R%d]\n", dataReg, addrReg);
			releaseRegister(addrReg);
			return dataReg;
		...
		case Nmethodcall:
		{
			...
			// 1. Push self FIRST (deepest in stack)
			int selfReg = getFreeRegister();
			if (root->left == NULL) {
				// self-call inside a method: load self from stack
				int paramCount = 0;
				struct Lsymbol* l = LSThead;
				while (l) { if (l->binding < 0) paramCount++; l = l->next; }
				int selfBinding = -(paramCount + 3);
				fprintf(target_file, "MOV R%d, BP\n", selfReg);
				fprintf(target_file, "ADD R%d, %d\n", selfReg, selfBinding);
				fprintf(target_file, "MOV R%d, [R%d]\n", selfReg, selfReg);
			} else {
			...
			// 2. Push args in reverse order (so arg1 is at BP-3 ... wait, self is now at BP-3)
			//    After self: arg1 at BP-4, arg2 at BP-5, etc.
			for (int i = ac-1; i >= 0; i--) {
				int r = codeGen(args[i]);
				fprintf(target_file, "PUSH R%d\n", r);
				releaseRegister(r);
...
struct tnode* makeStringNode(char* str)
{
    ...
    temp->type = 2;
    temp->typeEntry = TLookup("str");   // ← add this
    temp->varname = NULL;
    temp->nodetype = Nstr;
    temp->left = NULL;
    temp->right = NULL;
...
tnode* makeClassDeclNode(tnode* varlist, Ctable* ctype) {
    // collect vars in order (left-leaning tree, same as makeDeclNode)
    tnode* vars[64];
    int vc = 0;
    tnode* tmp = varlist;
    while (tmp != NULL) {
    ...
    // install each class variable into GST
    for (int i = 0; i < vc; i++) {
        tnode* target = vars[i];
        if (target == NULL || target->varname == NULL) continue;
        if (Lookup(target->varname) != NULL) {
        ...
        Gsymbol* entry = InstallClass(target->varname, ctype, 1);
...
extern Ctable* current_class;

tnode* makeSelfFieldNode(char* fieldname) {
    if (current_class == NULL) {
        printf("Semantic Error: 'self' used outside of a class method\n");
        exit(1);
    }
    // check field exists in current class
    Tfield* field = CFLookup(current_class, fieldname);
    if (field == NULL) {
        printf("Semantic Error: Field %s not found in class %s\n",
               fieldname, current_class->name);
        exit(1);
    }
    tnode* node = (tnode*)malloc(sizeof(tnode));
    node->nodetype              = Nself;
    node->varname               = strdup(fieldname);
    node->typeEntry             = field->type;   // type of the field
    node->classEntry            = current_class; // which class self refers to
    node->type                  = 0;
    node->left                  = NULL;
    node->right                 = NULL;
    node->symbolTableEntry      = NULL;
...
tnode* makeSelfMethodCallNode(char* methodname, tnode* arglist) {
    if (current_class == NULL) {
        printf("Semantic Error: 'self' used outside of a class method\n");
        exit(1);
    }
    Cmethod* m = CMLookup(current_class, methodname);
    if (m == NULL) {
        printf("Semantic Error: Method %s not found in class %s\n",
               methodname, current_class->name);
        exit(1);
    }
    ...
    Param* fps[64];
    int fpc = 0;
    Param* fp = m->paramlist;
    while (fp) { fps[fpc++] = fp; fp = fp->next; }

    // collect actual args into array
    tnode* args[64];
    int ac = 0;
    tnode* tmp = arglist;
    while (tmp) {
    ...
    for (int i = 0; i < ac; i++) {
        Ttable* atype = NULL;
        if (args[i]->localSymbolTableEntry != NULL)
            atype = args[i]->localSymbolTableEntry->type;
        else if (args[i]->symbolTableEntry != NULL)
            atype = args[i]->symbolTableEntry->type;
        else
            atype = args[i]->typeEntry;
        if (atype != fps[i]->type) {
            printf("Semantic Error: Argument type mismatch in call to %s\n", methodname);
            exit(1);
        }
    ...
    tnode* node = (tnode*)malloc(sizeof(tnode));
    node->nodetype              = Nmethodcall;
    node->varname               = strdup(methodname);
    node->typeEntry             = m->returnType;
    node->classEntry            = current_class;
    node->type                  = 0;
    node->left                  = NULL;   // self — no explicit var node needed
    node->right                 = arglist;
    node->symbolTableEntry      = NULL;
    node->localSymbolTableEntry = NULL;
    node->isPointer             = 0;
    ...
    node->body                  = NULL;
    node->ldeclblock            = NULL;
    // store the method label for codegen
    node->val                   = m->flabel;
    return node;
}

tnode* makeObjMethodCallNode(tnode* obj, char* methodname, tnode* arglist) {
    // obj must be a class variable
    Ctable* objClass = NULL;

    if (obj->classEntry != NULL) {
        objClass = obj->classEntry;
    } else if (obj->symbolTableEntry != NULL && obj->symbolTableEntry->ctype != NULL) {
        objClass = obj->symbolTableEntry->ctype;
    } else {
        printf("Semantic Error: %s is not a class variable\n",
               obj->varname ? obj->varname : "?");
        exit(1);
    }

    // look up method in that class
    Cmethod* m = CMLookup(objClass, methodname);
    if (m == NULL) {
        printf("Semantic Error: Method %s not found in class %s\n",
               methodname, objClass->name);
        exit(1);
    }
    ...
    node->varname               = strdup(methodname);
    node->typeEntry             = m->returnType;
    node->classEntry            = objClass;
    node->type                  = 0;
    node->left                  = obj;     // the object variable
    node->right                 = arglist;
    node->symbolTableEntry      = NULL;
    node->localSymbolTableEntry = NULL;
    node->isPointer             = 0;
...
tnode* makeNewNode(char* classname) {
    Ctable* c = CLookup(classname);
    if (c == NULL) {
        printf("Semantic Error: Unknown class %s in new()\n", classname);
        exit(1);
    }
    tnode* node = (tnode*)malloc(sizeof(tnode));
    node->nodetype              = Nnew;
    node->varname               = strdup(classname);
    node->classEntry            = c;
    node->typeEntry             = NULL;
    node->type                  = 0;
    node->left                  = NULL;
...
tnode* makeDeleteNode(tnode* var) {
    // var must be a class variable
    if (var->classEntry == NULL &&
        (var->symbolTableEntry == NULL || var->symbolTableEntry->ctype == NULL)) {
        printf("Semantic Error: delete() requires a class variable\n");
        exit(1);
    }
    tnode* node = (tnode*)malloc(sizeof(tnode));
    node->nodetype              = Ndelete;
    node->type                  = 0;
    node->typeEntry             = NULL;
    node->classEntry            = NULL;
    node->left                  = var;
    node->right                 = NULL;
    node->varname               = NULL;
    node->symbolTableEntry      = NULL;
...
int genSelfFieldAddress(tnode* t) {
    // t is an Nself node
    // self's heap address is the last argument passed to the method
    // it is passed as an extra implicit first argument at the deepest
    // stack position

    // find field offset
    Ctable* c = t->classEntry;
    Tfield* field = CFLookup(c, t->varname);
    if (field == NULL) {
    ...
    // self is passed as an implicit argument
    // it sits just below the explicit parameters in the stack
    // We need to know how many explicit params this method has
    // Count params from current_class method
    Cmethod* m = CMLookup(c, t->varname);
    // Actually self address is always at a fixed position:
    // It is pushed as the LAST argument before the return slot
    // so it is at BP - (numParams + 3)
    // But we need to find which method we're in — use current_class
    // For now, get param count from LST
    int paramCount = 0;
    struct Lsymbol* l = LSThead;
    while (l != NULL) {
        if (l->binding < 0) paramCount++;
        l = l->next;
    }
    // self is at BP - (paramCount + 3)
    int selfBinding = -(paramCount + 3);

    int baseReg = getFreeRegister();
    fprintf(target_file, "MOV R%d, BP\n", baseReg);
    fprintf(target_file, "ADD R%d, %d\n", baseReg, selfBinding);
    // load self's heap address
    int ptrReg = getFreeRegister();
    fprintf(target_file, "MOV R%d, [R%d]\n", ptrReg, baseReg);
    releaseRegister(baseReg);
    // add field offset
