...
int codeGen(tnode* root)
{
	...
	switch (root->nodetype)
	{
		...
		case Nfdef :
		{
			...
			Param* pp = root->paramlist;
			while (pp) { currentFnParamCount++; pp = pp->next; }
		...
		case Nmethodcall:
		{
			...
			if (root->left == NULL) {
				// self-call: self is at BP-(currentFnParamCount + 3)
				int selfBinding = -(currentFnParamCount + 3);
				fprintf(target_file, "MOV R%d, BP\n", selfReg);
				fprintf(target_file, "ADD R%d, %d\n", selfReg, selfBinding);
				fprintf(target_file, "MOV R%d, [R%d]\n", selfReg, selfReg);
			} else {
...
int genSelfFieldAddress(tnode* t) {
    ...
    // self is at BP-(currentFnParamCount + 3)
    int selfBinding = -(currentFnParamCount + 3);
