...
int genAssignInstr(tnode* t)
{
    ...
    // Store word 0 (heap ptr or value)
    fprintf(target_file, "MOV [R%d], R%d\n", addressReg, evalExprReg);

    // If RHS is new() — also store VFT ptr in word 1
    if (t->right->nodetype == Nnew) {
        Ctable* c = t->right->classEntry;
        int vftBase = 4096 + (c->classIndex * 8);
        int vftReg = getFreeRegister();
        int addr2Reg = getFreeRegister();
        fprintf(target_file, "MOV R%d, %d\n", vftReg, vftBase);
        fprintf(target_file, "MOV R%d, R%d\n", addr2Reg, addressReg);
